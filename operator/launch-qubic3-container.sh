...
AQDROP_CLIENT_DIR=${AQDROP_CLIENT_DIR:-/home/balewski/aqdrop-client}
AQDROP_OPERATOR_IMAGE=${AQDROP_OPERATOR_IMAGE:-${AQDROP_IMAGE:-registry.nersc.gov/dseg/aqdrop-operator:202608060}}
AQDROP_CONTAINER_RUNTIME=${AQDROP_CONTAINER_RUNTIME:-podman}
...
# ----  QubiC speciffic config  ----
DATA_VAULT=/dataVault2026
QUBIC_CALIB_BASE_PATH=/home/balewski/dataVault2026/qpus_calib
printf 'QUBIC_CALIB_BASE_PATH=%s\n' "$QUBIC_CALIB_BASE_PATH"
test -f "$QUBIC_CALIB_BASE_PATH/active_qpus.yaml" || { echo "Missing $QUBIC_CALIB_BASE_PATH/active_qpus.yaml"; exit 1; }
...
exec "$AQDROP_CONTAINER_RUNTIME" run --rm -it \
    ...
    --volume "$QUBIC_CALIB_BASE_PATH:/opt/qpus_calib" \
     --volume "/home/balewski$DATA_VAULT:$DATA_VAULT" \
    "$AQDROP_OPERATOR_IMAGE" \
